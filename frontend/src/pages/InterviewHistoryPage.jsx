...
import { getInterviewHistory, getDashboard } from "../services/dashboardApi";
import { downloadReportPdf } from "../services/oaApi";

/**
 * Interview History (/history).
 *
 * Lists every interview session for the logged-in user (one row per session,
 * never overwritten). Data comes from GET /api/oa/reports, scoped to the user.
 */
...
export default function InterviewHistoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(history);
      setActive(dash?.active_interview || null);
      setStatus("ready");
    } catch (err) {
      setErrorMsg(err.response?.data?.message || err.message || "Unable to load history.");
      setStatus("error");
    }
  }, []);
    ...
    <div className="min-h-screen px-4 sm:px-6 pt-6 pb-16">
      ...
      <div className="max-w-4xl mx-auto animate-fade-in-up">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-extrabold text-text-primary tracking-tight">Interview History</h1>
            <p className="text-text-secondary text-sm mt-1">All your interview sessions.</p>
          </div>
          <button onClick={() => navigate("/home")} className="text-xs px-3 py-2 rounded-xl text-text-secondary border border-white/10 hover:bg-white/5 cursor-pointer">
            ← Dashboard
          </button>
        </div>
        ...
        {status === "ready" && (
          active || rows.length ? (
            <div className="space-y-3">
              {active && (
                <div className="glass-strong rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-unverified/30">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-text-primary truncate">{active.resume_name || "Interview"}</p>
                    <p className="text-xs text-text-muted mt-0.5">Started {formatDate(active.started_at)}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs px-2 py-1 rounded-md text-unverified border border-unverified/30 bg-unverified/10">In Progress</span>
                    <button onClick={() => navigate("/dashboard")} className="text-xs px-3 py-1.5 rounded-md font-semibold text-white bg-gradient-to-r from-brand-500 to-brand-600 cursor-pointer">
                      Resume Interview
                    </button>
                  </div>
                </div>
              ...
              {rows.map((r) => (
                <div key={r.interview_id} className="glass-strong rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-text-primary truncate">{r.resume_name || "Interview"}</p>
                    <p className="text-xs text-text-muted mt-0.5">
                      {formatDate(r.generated_at)} · {(r.completed_rounds || []).join(", ") || "—"}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="glass-strong rounded-2xl p-10 text-center">
              <p className="text-text-secondary text-sm mb-5">You have no interview history yet.</p>
